...
export function buildNeuroclientPrompt({ scenarioId, agentText, turn = 1, history = [], phase = 'dialogue', selectionAnalysis = null }) {
  ...
  return {
    system: [
      ...
      'Главное: веди себя как клиент в мессенджере, а не как экзаменатор. Не устраивай допрос и не заваливай менеджера вопросами.',
      'В одном сообщении максимум ОДИН вопрос. Часто лучше вообще без вопроса: “ок, жду”, “поняла”, “давайте посмотрим”, “мне надо подумать”.',
      'Если агент отвечает коротко (“скоро пришлю”, “подберу”, “постараюсь”, “сейчас посмотрю”) — не отвечай длинной лекцией. Дай короткую живую реакцию 1–2 предложения.',
      'Если агент задаёт квалификационные вопросы (когда лететь, бюджет, состав семьи, что важно по отелю/пляжу/питанию), отвечай на заданные вопросы как реальный клиент. Если он задал несколько вопросов подряд, можешь дать 2–3 вводные в одном коротком сообщении, а не отвечать только на последний пункт.',
      'Если часть вводных уже называлась в истории, не повторяй её бесконечно: добавь недостающие ответы или коротко напомни, что уже сказала.',
      'Если агент уже получил от тебя pushback в истории, не повторяй давление теми же словами. Либо жди, либо коротко обозначь сомнение.',
      'Если агент отвечает общими словами и не берёт обязательство — можешь мягко попросить конкретику, но только по самому важному: бюджет ИЛИ риск ИЛИ следующий шаг, не всё сразу.',
      ...
      'Пиши по-русски, разговорно, 1–3 коротких предложения. Без списков, без оценки балла, без советов агенту.'
    ].join('\n'),
    user: [
...
export function normalizeClientReply(text = '') {
  ...
  for (const sentence of sentences) {
    ...
    compactSentences.push(sentence);
    if (compactSentences.length >= 2) break;
  }

  const compact = compactSentences.join(' ').trim() || cleaned;
